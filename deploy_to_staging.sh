...
#
# 使用方法:
#   ./deploy_to_staging.sh           # 默认：不重新构建，直接启动现有容器
#   ./deploy_to_staging.sh --rebuild # 强制重新构建镜像（耗时约20分钟）
# ============================================================================
...
local_checks() {
    ...
    # 1.7 检查现有 Docker 镜像
    print_step "检查现有 Docker 镜像..."
    if docker images | grep -q "pythondemo-web"; then
        print_success "发现现有 Docker 镜像"
        if [ "$REBUILD_IMAGES" = false ]; then
            print_info "将使用现有镜像启动容器（不重新构建）"
            print_info "如需重新构建，请使用: $0 --rebuild"
        else
            print_warning "将重新构建 Docker 镜像（耗时约20分钟）"
        fi
    else
        print_warning "未发现现有镜像，将进行首次构建"
        REBUILD_IMAGES=true
    fi
}
...
build_images() {
    if [ "$REBUILD_IMAGES" = false ]; then
        print_header "阶段 3: 跳过镜像构建"
        print_info "使用现有 Docker 镜像"
        print_success "跳过构建阶段"
        return 0
    fi
    ...
    print_warning "这可能需要约20分钟时间（安装 PyTorch 和依赖）..."
...
print_summary() {
    ...
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo "✅ 默认模式：使用现有镜像快速部署（推荐用于日常测试）"
    echo "   命令: ./deploy_to_staging.sh"
    echo ""
    echo "🔨 重建模式：重新构建镜像（仅在更新依赖或首次部署时使用）"
    echo "   命令: ./deploy_to_staging.sh --rebuild"
    echo "   注意: 重建需要约20分钟（安装 PyTorch 等大型依赖）"
    echo ""
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
main() {
    ...
    if [ "$REBUILD_IMAGES" = true ]; then
        echo -e "${YELLOW}模式: 重新构建镜像${NC}"
    else
        echo -e "${GREEN}模式: 使用现有镜像（快速部署）${NC}"
    fi
    echo ""
